...
const AppendixDocumentPage = (props: Props): JSX.Element => {
    ...
    const appendixURL = router.asPath && MakeSocialUrl(router.asPath, props.navData.domain) || props.navData.domain;

    if (props.document == undefined) {
        return <NotFoundPage requestedItem={`Section: ${requestedRes}`} />
    }

    const documentProps = CreateEPubReader(props.document, 
        appendixURL, 
        props.facebookAppId, 
    ...
    return (
        <Layout navData={props.navData} backgroundImageUrl={props.document.metadata.background_image} socials={socialData} isReadingView={true}>
            <EPubReaderPage {...documentProps}></EPubReaderPage>
        </Layout>
    );
};
